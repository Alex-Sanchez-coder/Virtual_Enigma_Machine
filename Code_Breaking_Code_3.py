...
### Rotors: UNKNOWN, the lis of rotors to use is II, IV, Beta / Gamma, 0 value to not include a 4 rotor   (THIS PART IS ITERATED BY THE CODE)
### Reflector: UNKNOWN, the list includes A, B and C  (THIS PART IS ITERATED BY THE CODE)
### Ring settings: UNKNOWN, the list includes odd numbers except 12  and 0 value to not include a 4 rotor    (THIS PART IS ITERATED BY THE CODE)
### Ring List=[02, 04, 06, 08, 10, 14, 16, 18, 20, 22, 24] for all rotors   (DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
### Starting positions: E M Y and 0 value to not include a 4 rotor    (DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
### Plugboard pairs: FH TS BE UQ KD AL   (DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
### The code introduced was: ABSKJAKKMRITTNYURBJFWQGRSGNNYJSDRYLAPQWIAGKJYEPCTAGDCTHLCDRZRFZHKNRSDLNPFPEBVESHPY (DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
### crib: THOUSANDS  (DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
...
#___________________________________________rotor permutations___________________________________________________________________
# Creating all combinations with the given rotors
def rotor_permutations(Rotor_list):
    comb = itertools.permutations(Rotor_list, 3)
...
def decrypting(Rotor_list, Reflector_list, Ring_setting_list, LHS_position_setting, Middle_position1_setting,
            Middle_position2_setting, RHS_position_setting, encoded_message, crib):

    # To get all combinations in rotors
    Rotor_list = rotor_permutations(Rotor_list)

    # Changing rotors thru all combinations
    for n in range(len(Rotor_list)):
        # Changing reflectors (3 of them)
        for m in range(len(Reflector_list)):
            # Changing rotors ring settings (3 of them)
            for k in range(len(Ring_setting_list)):
                for j in range(len(Ring_setting_list)):
                    for i in range(len(Ring_setting_list)):

                        #print(f"LHS rotor: {Rotor_list[n][0]}, Middle rotor: {Rotor_list[n][1]}, RHS rotor: {Rotor_list[n][2]}")
                        #print(f"Reflector: {Reflector_list[m]}")
                        #print(f"LHS rotor´s ring setting: {Ring_setting_list[k]}, Middle rotor´s ring setting: {Ring_setting_list[j]}, RHS rotor´s ring setting: {Ring_setting_list[i]}")

                        # Saving the result in a list
                        encoded_message_proposal = enigma_machine_set_up(Rotor_list[n][0], Rotor_list[n][1], Rotor_list[n][2], 0, Reflector_list[m], Ring_setting_list[k], Ring_setting_list[j],
                            Ring_setting_list[i], 0, LHS_position_setting, Middle_position1_setting,
                        ...
                        if crib in encoded_message_proposal_string:
                            print(f"The LHS rotor is: {Rotor_list[n][0]}")
                            print(f"The Middle rotor is: {Rotor_list[n][1]}")
                            print(f"The RHS rotor is: {Rotor_list[n][2]}")
                            print(f"The reflector is: {Reflector_list[m]}")
                            print(f"The original LHS rotor´s ring setting is: {Ring_setting_list[k]}")
                            print(f"The original Middle rotor´s ring setting is: {Ring_setting_list[j]}")
                            print(f"The original RHS rotor´s ring setting is: {Ring_setting_list[i]}")
...
plugboard_set_up("FH TS BE UQ KD AL")

#Insert here the encoded message_____________(DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
encoded_message = "ABSKJAKKMRITTNYURBJFWQGRSGNNYJSDRYLAPQWIAGKJYEPCTAGDCTHLCDRZRFZHKNRSDLNPFPEBVESHPY"
...
# II, IV, Beta / Gamma, Reflector_list, Ring_setting_list, "E", "M", "Y",
Rotor_list=["II", "IV", "Beta", "Gamma"]

# Ring settings: (DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
Ring_setting_list = ["02", "04", "06", "08", "20", "22", "24", "26"]

# Position settings: (DO NOT MODIFY ONLY IF THE SET UP IS DIFFERENT)
LHS_position_setting = "E"
Middle_position1_setting = "M"
Middle_position2_setting = "Y"
RHS_position_setting = 0

# Function to iterate and find the answers
decrypting(Rotor_list, Reflector_list, Ring_setting_list, LHS_position_setting, Middle_position1_setting,
            Middle_position2_setting, RHS_position_setting, encoded_message, crib)
